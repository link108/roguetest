...
class Fighter:
  ...
  def attack(self, target, state):
    ...
    if damage > 0:
      ...
      state.status_panel.message(
        self.owner.name.capitalize() + ' attacks ' + target.name + ' for ' + str(damage) + ' hit points', libtcod.white)
      target.fighter.take_damage(damage, state)
    else:
      state.status_panel.message(self.owner.name.capitalize() + ' attacks ' + target.name + ' but it has no effect!',
                                 libtcod.gray)
  ...
  def get_info(self, state):
      return "max hp: " + str(self.max_hp(state)) + ", current hp: " + str(self.hp) + ", power: " + str(self.power(state)) + ", defense: " + str(self.defense(state))
