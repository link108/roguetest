...
class HighScores:
  ...
  def show_high_scores(self, state, menu):
    ...
    index = menu.display_menu_return_index('High Score list', high_scores_to_display, MapConstants.MAP_WIDTH, state.con,
                                           override_height=MapConstants.MENU_HEIGHT_ADDITION)
    if index:
      chosen_high_score = high_scores[index]
      menu.display_menu_return_index('High Score of ' + chosen_high_score.name, [str(chosen_high_score)],
                                     MapConstants.MAP_WIDTH, state.con, override_height=20, option_char=False)
